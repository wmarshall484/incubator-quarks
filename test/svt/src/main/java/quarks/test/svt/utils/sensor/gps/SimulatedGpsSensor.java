...
public class SimulatedGpsSensor {
	...
	private int requestedIterations = -1; // -1 means loop through gpsDataArray forever
	...
	public GpsSensor nextGps() {
		...
		if (requestedIterations == -1 || currentIteration < requestedIterations ) {
			...
			// hardcode other data (altitude, course, time, speed, etc.) and at index 5 for hard braking and acceleration speed
			if (currentIndex == 5) 					
				result = new GpsSensor(gpsDataArray[currentIndex][0],  gpsDataArray[currentIndex][1], 32.1, 42.4, simulatedSec, 38.2 );
			else 
				result = new GpsSensor(gpsDataArray[currentIndex][0],  gpsDataArray[currentIndex][1], 20.1, 29.0, simulatedSec, 28.2);
